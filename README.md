...
直接把 patch.jar 路径作为第 3 个参数传给 cli（不再需要 hotfix-config.json）：
...
> **路径解析**：`agent.jar` / `patch.jar` 相对路径以 **cli 运行时的 CWD** 为基准解析为绝对路径后传给 agent。推荐 `cd` 进 hotfix 目录后全用相对参数；绝对路径也支持。
...
- **线程安全**：redefine 在 safepoint 完成类元数据替换，不等运行栈帧退出；`while(true)` 死循环方法 redefine 后运行线程永远拿不到新逻辑，避免热更 main loop 类
...
**Q: agent 报 "patch jar not found" 且路径是乱码？**
A: Attach API 传输 `agentArgs`（patchJar 路径）时非 ASCII 路径会乱码。**patchJar 路径需用 ASCII**（生产 `/opt/gameserver/` 天然满足；本地验证若工作区含中文，把产物拷到 `C:\hotfix-test\` 之类的 ASCII 目录再跑）。
